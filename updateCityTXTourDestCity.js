...
var txVocName = ['City','Tour Destination'];
...
var dataValidation = () => {
	var txCityValidationLog = '', txTDValidationLog = '';
	var citiesJson = [], destJson = [];
			...
			txVocName.forEach( (vocName) => {
				...
				if(!txTermsId[vocKey][ctn.text]){
					...
					if(vocKey === 'City'){
						...
						txTDValidationLog += ctn.text + '\n';
						var destAddFlag = true;
						destJson.forEach( (dest) => {
							if(dest.Title === ctn.text)
								destAddFlag = false;
						});
						if(destAddFlag)
							destJson.push({"Title":ctn.text});
					}
				}
			});
		});
