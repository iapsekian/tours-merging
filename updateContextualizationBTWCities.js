...
var txVocName = ['City'];
var ctnTypeName = ['City', 'City Details'];
		...
		ctns.forEach( (ctn) => {
			...
			txVocName.forEach( (vocName) => {
				...
				if(!txTermsId[vocKey][ctn.text]){
					...
					if(addFlag)
						citiesJson.push({"Title":ctn.text});
				}
			});
		});
	});	

	fs.writeFileSync('./logs/notExistedInTaxonomyCities-' + targetEnv + '.log', txValidationLog);
		...
		dbConnection.then( (db) => {
			...
			allContents.forEach( (key) => {
				...
				var wait4ctnsEnd = () => {
					ctnsCount--;
					if(!ctnsCount){
						fs.writeFileSync('./logs/updateContextualizationBTWCities-'+key+'-'+targetEnv+'.log', ctnsUpdLog);
						fs.writeFileSync('./logs/updateContextualizationBTWCities-'+key+'-'+targetEnv+'.json', JSON.stringify(result));
						wait4AllContentsEnd();
					}
				}
...
var endProgram = () => {
	console.log('*** updateContextualizationBTWCities.js Finished!! ***');	
}
